...
/**
 * Converter class for preparing the Browse Paths v.2 aspect
 */
public class KeyspaceToBrowsePathsV2Converter extends KeyspaceToAspectConverter<BrowsePathsV2>
{
    public KeyspaceToBrowsePathsV2Converter(@NotNull final IdentifiersProvider identifiers)
    {
        super(identifiers);
    }
    ...
    @Override
    @NotNull
    public MetadataChangeProposalWrapper<BrowsePathsV2> convert(@NotNull final KeyspaceMetadata keyspace)
    {
        final String urn = identifiers.urnContainer(keyspace);

        final BrowsePathsV2 aspect = new BrowsePathsV2().setPath(new BrowsePathEntryArray(
                new BrowsePathEntry()
